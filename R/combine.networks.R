combine.networks <- function(nets, contributions, outPath, midfix="",powerVector=1:20,
    ...
    if(FALSE){
        ...
        maxBlockSize <- nrow(network2)
        ##power
        pstPower <- WGCNA::pickSoftThreshold.fromSimilarity(similarity=network2,
        ...
        TOM <- as.dist(WGCNA::TOMsimilarity(adjMat=network2^pstPower$powerEstimate,
        ...
        if(verbose>1)
            print(table(modules))
        names(modules) <- rownames(network2)
